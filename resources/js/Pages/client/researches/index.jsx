...
/**
 * Page listant l'ensemble des recherches (paginées côté backend).
 * Attendu depuis le contrôleur (Inertia::render):
 * - researches: objet de pagination Laravel { data: [...], links: [...], meta: {...} }
 *   ou simplement un tableau si vous n'utilisez pas de pagination.
 */
const ResearchesIndex = ({ researches }) => {
  const headerRef = useRef(null);
  const items = Array.isArray(researches) ? researches : researches?.data ?? [];
  const links = researches?.links ?? researches?.meta?.links ?? null;
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-10 relative">
          ...
          >
            ...
            <h1 className="font-display text-3xl sm:text-4xl lg:text-5xl text-[#1f2d2d] font-medium leading-tight">
              All our <span className="text-[#bf5429]">research</span> studies
            </h1>
          </div>
          ...
          {items.length === 0 ? (
            <div className="text-center py-24">
              <p className="text-[#5f6967]">No research publications yet.</p>
            </div>
          ) : (
          ...
          {/* Pagination (si researches est un objet paginé Laravel) */}
...
export default ResearchesIndex;